...
This document defines the test cases and testing scenarios for validating the Forward Proxy Server and Management Console.
...
### Functional Proxy Verification (CLI and API)

| Test Case ID | Test Case Title | Pre-Conditions | Execution Steps | Expected Result |
| :--- | :--- | :--- | :--- | :--- |
| **TC-FP-01** | Verify HTTP GET Forwarding | Docker containers running; `httpbin.org` accessible | Run `curl.exe -x http://localhost:8888 http://httpbin.org/ip` | Returns HTTP 200 containing client origin IP. Logs are written to MongoDB. |
| **TC-FP-02** | Verify HTTPS CONNECT Tunneling | Docker containers running; `google.com` accessible | Run `curl.exe -x http://localhost:8888 https://www.google.com` | Connection establishes successfully. TLS handshake completes; returns HTML. |
| **TC-FP-03** | Verify Domain Blocking (HTTP) | `example.com` added to blocklist rule | Run `curl.exe -i -x http://localhost:8888 http://example.com` | Returns `HTTP/1.1 403 Forbidden` with body: "Access to example.com is blocked by proxy policy." |
| **TC-FP-04** | Verify Domain Blocking (HTTPS CONNECT) | `badsite.com` added to blocklist rule | Run `curl.exe -i -x http://localhost:8888 https://badsite.com` | Returns `HTTP/1.1 403 Forbidden` and client connection is terminated. |
...
| **TC-UI-03** | Test Non-Blocked URL in UI | 1. Enter `https://httpbin.org/ip` in testing URL field<br>2. Click **Test via Proxy** | Status `200` is displayed along with headers, response preview, and total length. |
